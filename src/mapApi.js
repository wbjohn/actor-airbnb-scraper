...
async function findPolygons(
  { query, street, city, country, county, state, postalcode },
  getRequest
) {
  const additionalQuery = { polygon_geojson: 1, format: "json" };
  const urlQuery = objectToQuery({
    ...additionalQuery,
    q: query,
    street,
    city,
...
async function cityToAreas(
  ...
  const filteredPolygons = polygons.filter((polygon) => {
    ...
    if (polygon.importance && polygon.importance <= MIN_IMPORTANCE)
      return false;
  ...
  if (!pointMap.size) {
    for (const polygon of filteredPolygons) {
      log.info("Finding points in polygon, (might take a while)", {
        place_id: polygon.place_id,
      });
      const pips = findPointsInPolygon(
        polygon,
        distanceKilometers,
        limitPoints
      );
  ...
  for (const point of points) {
    promises.push(Promise.race([sleep(timeoutMs), reversePoint(point)]));
