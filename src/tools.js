...
const {
  utils: { log, requestAsBrowser, sleep },
} = Apify;
 ...
 */
const getRequestFnc =
  (session, proxy, locale = DEFAULT_LOCALE) =>
  async (url, opts = {}) => {
    const getData = async (attempt = 0) => {
      ...
      const options = {
        ...
        proxyUrl: proxy.newUrl(
          session
            ? session.id
            : `airbnb_${Math.floor(Math.random() * 100000000)}`
        ),
        abortFunction: (res) => {
...
const getStartRequestsFromUrl = async (startUrl) => {
  ...
  if (
    ...
    const [googlesheetLink] = startUrl.requestsFromUrl.match(
      /.*\/spreadsheets\/d\/.*\//
    );
...
const enqueueDetailRequests = async (
  requestQueue,
  startUrls,
  { minPrice, maxPrice }
) => {
  ...
  log.info(
    `Starting with ${startRequests.length} request${
      startRequests.length !== 1 ? "s" : ""
    }`
  );
  ...
  while (request) {
    if (
      !request.url.match(RegExp(URL_WITH_ROOMS_REGEX)) &&
      !request.url.includes("abnb.me/")
    ) {
    ...
    const detailRequest = buildDetailRequest(
      id,
      minPrice,
      maxPrice,
      request.url,
...
const enqueueLocationQueryRequests = async (
  requestQueue,
  input,
  proxy,
  buildListingUrlFnc
  ...
  if (!maxListings || maxListings > 1000) {
    ...
    const cityQuery = await getSearchLocation(
      { maxPrice, minPrice },
      locationQuery,
      doReq,
      buildListingUrlFnc
 ...
 */
async function enqueueListingsFromSection(
  results,
  requestQueue,
  minPrice,
  maxPrice,
  ...
  for (const result of results) {
    const {
      rate,
      rate_type: rateType,
      rate_with_service_fee: rateWithServiceFee,
    } = get(result, ["pricing_quote"], {});

    const detailLink = buildDetailRequest(
      result.listing.id,
      minPrice,
      maxPrice,
      originalUrl,
      { rate, rateType, rateWithServiceFee }
    );
 ...
 * @param {(...args: any) => string} buildListingUrl
 */
async function getListingsSection(
  { minPrice, maxPrice },
  location,
  requestQueue,
  getRequest,
  ...
  if (listings.length) {
    await enqueueListingsFromSection(
      listings,
      requestQueue,
      minPrice,
      maxPrice,
  ...
  log.debug(
    `Listings metadata: listings: ${listings.length}, hasNextPage: ${hasNextPage}`
  );
  ...
  while (hasNextPage) {
    ...
    // eslint-disable-next-line camelcase
    const {
      pagination_metadata: nextPaginationMetadata,
      sections: nextSections,
    } = nextData.explore_tabs[0];
...
async function pivot(request, requestQueue, getRequest, buildListingUrl) {
  ...
  if (listingCount > 1000 && pivotEnd - pivotStart > 1) {
    ...
    await getListingsSection(
      { minPrice: pivotStart, maxPrice: pivotEnd },
      query,
      requestQueue,
      getRequest,
      buildListingUrl
...
const getFormattedUser = (apiUser) => {
  return {
    ...
    thumbnailUrl: apiUser.pictureUrl.replace(
      "profile_x_medium",
      "profile_small"
    ),
  };
...
async function getReviews(listingId, getRequest, maxReviews) {
  ...
  try {
    ...
    for (let i = 0; i < numberOfFetches; i++) {
      offset += pageSize;
      await randomDelay();
      (await req()).reviews.forEach((rev) =>
        results.push(getFormattedReview(rev))
      );
...
async function getCalendar(request, detailId, checkIn, calendarMonths, doReq) {
  ...
  try {
    const checkInDate =
      (originalUrl
        ? new URL(originalUrl, "https://www.airbnb.com").searchParams.get(
            "check_in"
          )
        : false) ||
      checkIn ||
    ...
    log.info(`Requesting calendar for ${checkInDate}`, {
      url: request.url,
      id: detailId,
    });
    const { calendar_months: months } = await doReq(
      getCalendarMonths(detailId, checkInDate, calendarMonths)
    );
    ...
    log.exception(e, "Error while retrieving calendar", {
      url: request.url,
      id: detailId,
    });
  }
...
function makeInputBackwardsCompatible(input) {
  ...
  if (input.includeCalendar) {
    log.warning(
      'The "includeCalendar" input parameter is deprecated and will be removed in the future. ' +
...
function validateInput(input) {
  ...
  if (!input.locationQuery && input.startUrls.length <= 0) {
    throw new Error(
      "At least one of the 'locationQuery' or 'startUrls' should be present."
    );
  }
